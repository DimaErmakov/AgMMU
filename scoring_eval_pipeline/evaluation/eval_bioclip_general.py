...
def run_llms(prompt, img, q):
    ...
    model_path = "/u/dermakov/clip_sf_epoch_19.pth"
    import torch
    try:
...
def eval_data(data, llm_map, output, image_dir):
    ...
    for q in data:
        ...
        faq_id = q["faq-id"]
        #TODO, I need to find the top 10 based on qid for that qid and then I need to add the top 10 to the prompt
...
def main():
    ...
    eval_data(
        data=data,
        llm_map={},  # Updated model names
        output=args.output_path,
        image_dir=args.image_dir,
    )

# python eval_clip.py /u/dermakov/clip_sf_epoch_19.pth /u/dermakov/AgMMU/data/6k_evalset_wbg.json #CLIP
# python eval_clip.py /u/dermakov/clip_sf_large.pth /u/dermakov/AgMMU/data/6k_evalset_wbg.json #general
